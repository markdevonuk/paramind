...
function generateWaveform(rhythm, rate, width) {
    ...
    for (let x = 0; x < width; x++) {
        ...
        if (rhythm === 'ventricularFibrillation') {
            ...
            // Rate varies cyclically (simulating breathing)
            const breathCycle = Math.sin(adjustedX * 0.003) * 0.3;
            const variedInterval = beatInterval * (1 + breathCycle);
            let cyclePos = (adjustedX % variedInterval) / variedInterval;
            y = generatePQRST('normalSinus', cyclePos, baselineY, adjustedX, variedInterval);
        } else {
...
function generateAFBeat(phase, baselineY) {
    ...
    if (phase >= 0.0 && phase < 0.1) {
        y = baselineY + Math.sin((phase / 0.1) * Math.PI) * 4;
    }
    else if (phase >= 0.1 && phase < 0.25) {
        y = baselineY - Math.sin(((phase - 0.1) / 0.15) * Math.PI) * 60;
    }
    else if (phase >= 0.25 && phase < 0.35) {
        y = baselineY + Math.sin(((phase - 0.25) / 0.1) * Math.PI) * 10;
    }
    else if (phase >= 0.5 && phase < 0.85) {
        y = baselineY - Math.sin(((phase - 0.5) / 0.35) * Math.PI) * 12;
    }
    else {
        y = baselineY;
    }
...
function generatePQRST(rhythm, cyclePos, baselineY, adjustedX, beatInterval) {
    ...
    switch (rhythm) {
        ...
        case 'atrialFlutter':
            ...
            let flutterWave;
            if (flutterPos < 0.7) {
                flutterWave = baselineY + 10 - (flutterPos / 0.7) * 20;
            } else {
                flutterWave = baselineY - 10 + ((flutterPos - 0.7) / 0.3) * 20;
            }
            
            y = flutterWave;
            
            if (cyclePos >= 0.18 && cyclePos < 0.21) {
                y = baselineY - Math.sin(((cyclePos - 0.18) / 0.03) * Math.PI) * 55;
            }
            else if (cyclePos >= 0.21 && cyclePos < 0.24) {
                y = baselineY + Math.sin(((cyclePos - 0.21) / 0.03) * Math.PI) * 10;
            }
            break;
        ...
        case 'pvcs':
            ...
            if (pvcLocalPhase >= 0 && pvcLocalPhase <= 1) {
                if (isPVCBeat) {
                    ...
                    if (pvcLocalPhase >= 0.05 && pvcLocalPhase < 0.18) {
                        // Wide tall R wave
                        y = baselineY - Math.sin(((pvcLocalPhase - 0.05) / 0.13) * Math.PI) * 75;
                    } else if (pvcLocalPhase >= 0.18 && pvcLocalPhase < 0.32) {
                        // Deep wide S wave
                        y = baselineY + Math.sin(((pvcLocalPhase - 0.18) / 0.14) * Math.PI) * 30;
        ...
        case 'stemi':
            ...
            } else if (cyclePos >= 0.24 && cyclePos < 0.55) {
                // ST elevation — stays elevated above baseline
                const stBase = baselineY - 18;
                if (cyclePos < 0.35) {
                    // ST segment — elevated plateau
                    y = stBase;
                } else {
                    // Elevated T wave merging with ST segment
                    y = stBase - Math.sin(((cyclePos - 0.35) / 0.20) * Math.PI) * 12;
                }
            }
            break;
